...
function HtmlQuiz() {
  ...
  function changeQue() {
    if (queIndex < HtmlQuestion.length - 1) {
  ...
  function updateScore() {
    if (trackClick === HtmlQuestion[queIndex].ans) {
      ...
      <div className="container-fluid">
        <div className="row">
          ...
          <div className="score-section d-flex">
            <Scoreboard allQuestion={HtmlQuestion.length} nowScore={changeScore} />
            <>
              <div className="d-flex m-2 w-100 rounded border justify-content-center align-items-center">
                ...
                <span className="name d-block mx-2 fs-2 ">Html Language</span>
              </div>
            </>
          </div>
        </div>
            ...
            <div className="col-12">
              <div className="mb-5">
                <span className="d-block fs-2  w-100 text-center">
                  {queIndex + 1}.{HtmlQuestion[queIndex].question}
                </span>
              </div>
            </div>
            <div className="col-12">
              <div className="w-100 d-flex flex-column justify-content-center align-items-center">
                {HtmlQuestion[queIndex].option.map((option, i) => {
                  return (
                    <button
                    key={i} 
